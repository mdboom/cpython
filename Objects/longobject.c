...
{
    return v->ob_digit + (v->ob_digit[0] >> PyLong_LONG_FLAG_SHIFT);
}
...
static inline void
copy_digits(PyLongObject *a, size_t a_offset, PyLongObject *b, size_t b_offset, int ndigits)
{
    memcpy(get_digit_offset(a) + a_offset, get_digit_offset(b) + b_offset, sizeof(digit) * ndigits);
}

static inline void
clear_digits(PyLongObject *ob) {
    if (!_PyLong_IsCompact(ob)) {
        Py_ssize_t size = _PyLong_NonCompactDigitCount(ob);
        memset(ob->ob_digit + PyLong_HEADER_SIZE, 0, size * sizeof(digit));
    }
}
...
// MGDTODO: I didn't really understand the old calculation
#define MAX_LONG_DIGITS PyLong_MASK
...
_PyLong_New(Py_ssize_t size)
{
    ...
    if (ndigits == 1) {
        ...
        sizeof(digit)*(size + 1).  Previous incarnations of this code used
        sizeof() instead of the offsetof, but this risks being
        incorrect in the presence of padding between the header
        and the digits. */
        result = PyObject_Malloc(offsetof(PyLongObject, ob_digit) +
                                 (ndigits + PyLong_HEADER_SIZE)*sizeof(digit));
        if (!result) {
            PyErr_NoMemory();
            return NULL;
        }
        _PyObject_Init((PyObject*)result, &PyLong_Type);
        result->ob_digit[0] = PyLong_LONG_FLAG | size;
        result->ob_digit[1] = 0;
        return result;
    }
}
...
_PyLong_FromMedium(sdigit x)
{
    ...
    v->ob_digit[0] = (
        ((x < 0) ? PyLong_NEGATIVE_FLAG : 0) |
        (((x < 0) ? -x : x) & PyLong_MASK)
    );
    return (PyObject*)v;
}

// MGDTODO: No longer used internally -- remove?
PyLongObject *
_PyLong_FromDigits(int negative, Py_ssize_t digit_count, digit *digits)
{
    ...
    if (digit_count == 1) {
        result->ob_digit[0] = (
            (negative ? PyLong_NEGATIVE_FLAG : 0) |
            (digits[0] & PyLong_MASK)
        );
    } else {
        _PyLong_SetSignAndDigitCount(result, negative?-1:1, digit_count);
        memcpy(&result->ob_digit[PyLong_HEADER_SIZE], digits, digit_count * sizeof(digit));
    }
    return result;
}
...
_PyLong_Copy(PyLongObject *src)
{
    ...
    memcpy(result->ob_digit, src->ob_digit, (size + PyLong_HEADER_SIZE) * sizeof(digit));
    return (PyObject *)result;
}
...
_PyLong_FromLarge(stwodigits ival)
{
    ...
    if (v != NULL) {
        ...
        digit *p = v->ob_digit + PyLong_HEADER_SIZE;
        if (sign == -1) {
...
PyLong_FromDouble(double dval)
{
    ...
    for (i = ndig; --i >= 0; ) {
        ...
        v->ob_digit[i + PyLong_HEADER_SIZE] = bits;
...
PyLong_AsLongAndOverflow(PyObject *vv, int *overflow)
{
    ...
    *overflow = 0;
    ...
    if (_PyLong_IsCompact(v)) {
        ...
        while (--i >= 0) {
            prev = x;
            x = (x << PyLong_SHIFT) | v->ob_digit[i + PyLong_HEADER_SIZE];
...
_PyLong_Sign(PyObject *vv)
{
    ...
    Py_ssize_t negate = (v->ob_digit[0] >> PyLong_NEGATIVE_FLAG_SHIFT) & 1;
...
_PyLong_NumBits(PyObject *vv)
{
    ...
    assert(_PyLong_IsCompact(v) || v->ob_digit[ndigits - 1 + PyLong_HEADER_SIZE] != 0);
    if (ndigits > 0) {
       ...
       8-bit bytes and (probably) 15-bit or 30-bit Python digits.*/
    {
        ...
        for (i = 0; i < numsignificantbytes; ++i, p += incr) {
            ...
            if (accumbits >= PyLong_SHIFT) {
                ...
                vp[idigit] = (digit)(accum & PyLong_MASK);
                ++idigit;
                accum >>= PyLong_SHIFT;
                    ...
                    int little_endian, int is_signed)
{
    ...
    digit *vp = get_digit_offset(v);
    for (i = 0; i < ndigits; ++i, ++vp) {
        digit thisdigit = extract_digit(vp);
        if (do_twos_comp) {
...
rem1(PyLongObject *a, digit n)
{
    ...
    const Py_ssize_t size = _PyLong_NonCompactDigitCount(a);

    assert(n > 0 && n <= PyLong_MASK);
    return (PyLongObject *)PyLong_FromLong(
        (long)inplace_rem1(a->ob_digit + PyLong_HEADER_SIZE, size, n)
    );
}
    ...
    do {                                                                \
        if (size_a == 0) {                                              \
            *--p = '0';                                                 \
            ...
            Py_ssize_t i;                                               \
            for (i = 0; i < size_a; ++i) {                              \
                accum |= (twodigits)extract_digit(pa + i) << accumbits; \
                accumbits += PyLong_SHIFT;                              \
...
long_from_binary_base(const char *start, const char *end, Py_ssize_t digits, int base, PyLongObject **res)
{
    ...
    accum = 0;
    bits_in_accum = 0;
    pdigit = pstart = get_digit_offset(z);
    p = end;
    while (--p >= start) {
        int k;
        if (*p == '_') {
...
long_from_non_binary_base(const char *start, const char *end, Py_ssize_t digits, int base, PyLongObject **res)
{
    ...
    while (p < end) {
        ...
        if (c) {
            ...
            if (ndigits < size_z) {
                ...
                assert(ndigits + 1 <= buffer_size);
                ++ndigits;
                *pz = (digit)c;
            } else {
                PyLongObject *tmp;
                /* Extremely rare.  Get more space. */
                assert(ndigits == size_z);
                assert(size_z >= 1);
                #ifdef Py_DEBUG
                ...
                memcpy(tmp->ob_digit + 1,
                       ...
                       sizeof(digit) * size_z);
                Py_SETREF(z, tmp);
                z->ob_digit[size_z + PyLong_HEADER_SIZE] = c;
                ++ndigits;
                ++size_z;
            }
        }
    ...
    _PyLong_ModifyDigitCount(z, ndigits);
    *res = z;
    return 0;
}
...
_PyLong_Frexp(PyLongObject *a, Py_ssize_t *e)
{
    ...
    */
    if (a_bits <= DBL_MANT_DIG + 2) {
        ...
        x_size = shift_digits;
        assert(!_PyLong_IsCompact(a));
        rem = v_lshift(x_digits + x_size, a->ob_digit + PyLong_HEADER_SIZE, a_size,
                       (int)shift_bits);
        x_size += a_size;
        x_digits[x_size++] = rem;
    }
        ...
        assert(!_PyLong_IsCompact(a));
        rem = v_rshift(x_digits, a->ob_digit + PyLong_HEADER_SIZE + shift_digits,
        ...
        if (rem)
            ...
            while (shift_digits > 0)
                if (a->ob_digit[--shift_digits + PyLong_HEADER_SIZE]) {
                    x_digits[0] |= 1;
                    break;
                }
    }
...
long_compare(PyLongObject *a, PyLongObject *b)
{
    ...
    if (sign == 0) {
        ...
        while (--i >= 0) {
            diff = (sdigit)extract_digit(pa + i) - (sdigit)extract_digit(pb + i);
            if (diff) {
                break;
            }
        }
...
long_hash(PyLongObject *v)
{
    ...
    while (--i >= 0) {
        ...
        x += v->ob_digit[i + PyLong_HEADER_SIZE];
...
x_sub(PyLongObject *a, PyLongObject *b)
{
    ...
    if (size_a < size_b) {
        ...
        if (size_a <= 1) {
            digit aval, bval;
            aval = a->ob_digit[0] & PyLong_MASK;
            bval = a->ob_digit[0] & PyLong_MASK;
            if (aval < bval) {
                return (PyLongObject *)PyLong_FromLong(-(long)(bval - aval));
            } else {
                return (PyLongObject *)PyLong_FromLong(aval - bval);
            }
        }
        else {
            /* Find highest digit where a and b differ: */
            i = size_a;
            while (--i >= 0 && a->ob_digit[i + PyLong_HEADER_SIZE] == b->ob_digit[i + PyLong_HEADER_SIZE])
                ;
            if (i < 0)
                return (PyLongObject *)PyLong_FromLong(0);
            if (a->ob_digit[i + PyLong_HEADER_SIZE] < b->ob_digit[i + PyLong_HEADER_SIZE]) {
                sign = -1;
                { PyLongObject *temp = a; a = b; b = temp; }
            }
            size_a = size_b = i+1;
...
x_mul(PyLongObject *a, PyLongObject *b)
{
    ...
    if (a == b) {
         ...
         */
        assert(!_PyLong_IsCompact(a));
        digit *paend = a->ob_digit + size_a + PyLong_HEADER_SIZE;
        for (i = 0; i < size_a; ++i) {
            twodigits carry;
            twodigits f = a->ob_digit[i + PyLong_HEADER_SIZE] & PyLong_MASK;
            digit *pz = z->ob_digit + (i << 1) + PyLong_HEADER_SIZE;
            digit *pa = a->ob_digit + i + 1 + PyLong_HEADER_SIZE;
...
k_mul(PyLongObject *a, PyLongObject *b)
{
    ...
    memset(ret->ob_digit + PyLong_HEADER_SIZE, 0xDF, _PyLong_NonCompactDigitCount(ret) * sizeof(digit));
#endif
    ...
    copy_digits(ret, 2*shift, t1, 0, _PyLong_NonCompactDigitCount(t1));

    /* Zero-out the digits higher than the ah*bh copy. */
    i = _PyLong_NonCompactDigitCount(ret) - 2*shift - _PyLong_NonCompactDigitCount(t1);
    if (i)
        memset(ret->ob_digit + 2*shift + _PyLong_NonCompactDigitCount(t1) + PyLong_HEADER_SIZE, 0,
               i * sizeof(digit));
    ...
    if (i)
        memset(ret->ob_digit + _PyLong_DigitCount(t2) + PyLong_HEADER_SIZE, 0, i * sizeof(digit));
...
k_lopsided_mul(PyLongObject *a, PyLongObject *b)
{
    ...
    while (bsize > 0) {
        ...
        /* Add into result. */
        (void)v_iadd(ret->ob_digit + nbdone + PyLong_HEADER_SIZE, _PyLong_NonCompactDigitCount(ret) - nbdone,
                     product->ob_digit + PyLong_HEADER_SIZE, _PyLong_NonCompactDigitCount(product));
...
long_rshift1(PyLongObject *a, Py_ssize_t wordshift, digit remshift)
{
    ...
    digit *pa = a->ob_digit + PyLong_HEADER_SIZE;
    ...
    while ((size_a = _PyLong_DigitCount(a)) > 2) {
        ...
        */
        ...
        if (d != NULL) {
            ...
            if (d == NULL)
                goto error;
            clear_digits(d); // MGDTODO: Verified as needed
        }
...
long_subtype_new(PyTypeObject *type, PyObject *x, PyObject *obase)
{
    ...
    if (_PyLong_IsCompact(tmp)) {
        ...
        memcpy(newobj->ob_digit, tmp->ob_digit, (n + PyLong_HEADER_SIZE) * sizeof(digit));
    }
    Py_DECREF(tmp);
...
/*[clinic end generated code: output=2e571970daf1e5c3 input=7e0adef8e8ccdf2e]*/
{
    ...
    for (Py_ssize_t i = 0; i < ndigits_fast; i++) {
        bit_count += popcount_digit(extract_digit(pz + i));
    }
    ...
    for (Py_ssize_t i = ndigits_fast; i < ndigits; i++) {
        PyObject *x = PyLong_FromLong(popcount_digit(extract_digit(pz + i)));
        if (x == NULL) {
            goto error;
        }
